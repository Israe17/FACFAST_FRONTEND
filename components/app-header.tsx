...
function AppHeader() {
  ...
  const branchSummary = isBusinessLevelContext
    ...
    : activeBranchId
      ...
      : t("common.no_branch");
  const businessSummary = activeBusinessName ?? activeBusinessId ?? t("common.no_active_business");
  ...
  return (
    <header className="sticky top-0 z-30 flex h-16 shrink-0 items-center gap-2 border-b border-border/70 bg-background/80 px-4 backdrop-blur-xl">
      ...
      <div className="hidden items-center gap-2 xl:flex">
        ...
        {showTenantHeader ? <Badge variant="outline">{t("common.business_label", { label: businessSummary })}</Badge> : null}
        {showTenantHeader ? <Badge variant="outline">{t("common.branch_label", { label: branchSummary })}</Badge> : null}
        {isTenantContextMode ? (
          <Badge variant="outline">{t("common.platform_admin_in_tenant")}</Badge>
        ) : null}
        {isPlatformMode ? <Badge variant="outline">{t("common.no_tenant_context")}</Badge> : null}
      </div>
        ...
        {showPlatformEntryAction ? (
          <Button asChild variant="outline" size="sm">
            <Link href={APP_ROUTES.superadminEnterContext}>
              ...
              <span className="hidden sm:inline">{t("common.enter_business")}</span>
            </Link>
          </Button>
        ) : null}
