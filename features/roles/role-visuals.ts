...
export function pickRoleColor(roleKey: string | undefined, name: string): RoleColor {
  const key = (roleKey ?? "").toLowerCase();
  if (PRIVILEGED_KEYS.has(key)) {
    return ROLE_COLOR_PALETTE[0];
  }
  const seed = `${key}|${name}`;
  return ROLE_COLOR_PALETTE[hashString(seed) % ROLE_COLOR_PALETTE.length];
}

export function buildRoleInitials(name: string): string {
